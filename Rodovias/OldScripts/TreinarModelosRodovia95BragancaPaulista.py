...
RODOVIA_ALVO = "Rodovia Arão Sahm, KM 95 — Bragança Paulista (SP)."
BASE_DIR = Path(__file__).resolve().parent
ARQUIVO_BASE = BASE_DIR.parent / "DadosRodovias" / "dados_todas_rodovias.xlsx"
PASTA_SAIDA = r"modelos_rodovias\rodovia_arao_sahm_km_95_braganca_paulista_sp_rf_regimes"
...
def preparar_base(df):
    ...
    if "rodovia_identificacao" in df.columns:
        ...
        df = df[df["rodovia"] == "Rodovia Arão Sahm"].copy()
...
def treinar_modelo_braga(caminho_excel, pasta_saida):
    ...
    print("\n=== RESULTADO FINAL - BRAGANÇA PAULISTA ===")
