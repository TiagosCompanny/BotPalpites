...
RODOVIA_ALVO = "Floriano Rodrigues Pinheiro, KM 26 — Pindamonhangaba (SP)."
PASTA_SAIDA = r"modelos_rodovias\floriano_rodrigues_pinheiro_km_26_pindamonhangaba_sp_rf_regimes"
...
def preparar_base(df):
    ...
    if "rodovia_identificacao" in df.columns:
        df = df[df["rodovia_identificacao"] == RODOVIA_ALVO].copy()
    elif "rodovia" in df.columns:
        df = df[df["rodovia"].astype(str).str.contains("Floriano Rodrigues Pinheiro", case=False, na=False)].copy()

    if df.empty:
        raise ValueError(f"Nenhum registro encontrado para a rodovia: {RODOVIA_ALVO}")
...
def treinar_modelo_rodovia(caminho_excel, pasta_saida):
    ...
    bundle = {
        ...
        "kmeans": kmeans,
        "rodovia_identificacao": RODOVIA_ALVO
    }
